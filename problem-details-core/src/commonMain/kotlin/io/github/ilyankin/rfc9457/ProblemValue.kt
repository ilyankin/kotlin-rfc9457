 ...
 * String>`. Mirrors `kotlinx.serialization.json.JsonElement`: scalars keep their wire literal
 * verbatim, so a number survives both codecs byte for byte.
 *
...
/**
 * A scalar extension value — string, number or boolean.
 *
 * The wire literal is kept verbatim rather than parsed into a Kotlin type, which is what lets a
 * number survive both codecs byte for byte: `1.50` is written back as `1.50`, not as `1.5`.
 *
 * @property content the literal exactly as it appears on the wire, with the quotes of a JSON string
 *   already removed.
 * @property isString whether the value must be written as a quoted string — the only thing
 *   separating the string `"30"` from the number `30`, since `content` is `30` for both.
 */
public class ProblemPrimitive internal constructor(
    ...
    /**
     * Equal when both the literal and its string-ness match, so the string `"30"` and the number
     * `30` are different values despite sharing a [ProblemPrimitive.content] of `30`.
     */
 ...
 *
 * Note that JSON itself has one number type and no precision guarantee (RFC 8259 §6): a consumer
 * parsing into a `double` loses exactness past 2^53. Send an identifier that large as a string.
 */
 ...
 *
 * Has no XML representation — Appendix B defines nothing for `null` — so the XML codec writes it as
 * an empty element, which reads back as the empty string. A documented, tested asymmetry, not a bug.
 */
...
public class ProblemArray(
    ...
    /**
     * Equality of the contained values, against **any** `List` and not only another [ProblemArray] —
     * which is what `List` equality requires of a class that implements it, and what keeps
     * `ProblemArray(items) == items` true.
     */
...
public class ProblemObject(
    ...
    /**
     * Equality of the contained members, against **any** `Map` and not only another [ProblemObject],
     * for the same reason [ProblemArray.equals] compares against any `List`.
     */
 ...
 * member, and they are handled at different points —
 *
 ...
 * [primitiveOrNull], since §3 requires a wrong-typed member to be treated as absent rather than to
 * fail the document.
 *
 ...
 *
 * Combined with `extensions[name]?.`, this collapses three different situations — absent member,
 * explicit JSON null, wrong type — into the same `null`. Use `name in problem.extensions` to tell
 * absence apart from the other two.
 */
 ...
 *
 * Reads the wire *literal*, not the JSON type, so a quoted `"30"` also returns `30` — matching
 ...
 *
 * Strict: only the literals `true` and `false` are accepted, so `"TRUE"` and `1` both fail rather
