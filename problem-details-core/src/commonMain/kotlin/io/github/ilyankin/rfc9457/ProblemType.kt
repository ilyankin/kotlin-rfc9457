 ...
 *
 * [ProblemType.title] describes the *type*, so it is not overridable per problem — §3.1 asks it to
 * stay constant except for localization, which is out of scope for v1. Only the `detail` and
 * `instance` of [ProblemType.problem] vary per occurrence.
 *
...
public interface ProblemType {
     ...
     *
     * Named `typeUri` rather than `type` so that implementing this interface as an `enum class` or a
     * `sealed` hierarchy does not collide with a `type` in the implementor's own vocabulary. §3.1
     * recommends an absolute URI: a relative one resolves against the document's base URI, so it
     * means different things depending on where the document was fetched.
     */
